...
def plot_data(save_dir, episode_length=None, figure_title='', plot_filename='plot', perform_analysis=False):
    ...
    average_final_fidelity = np.mean(avg_final_fidelity_per_episode[-100:])
    if len(avg_final_fidelity_per_episode) >= 100:
        print("Average final fidelity over last 100 episodes", average_final_fidelity)
    ...
    fig.suptitle(figure_title + f" - average final fidelity: {average_final_fidelity}")
    fig.set_size_inches(10, 5)
